...
from typing import ClassVar, Type, Literal, Optional, Dict
from .ABCs import OpenMSMethodParam,OpenMSMethodConfig, MSToolConfig, MSTool, OpenMSDataWrapper
...
class MassTraceDetectionConfig(OpenMSMethodConfig):
    ...
    trace_termination_criterion: Literal["outlier", "sample_rate"] = Field(
        default="outlier",
        description="质量踪迹的终止准则。在'outlier'模式下，如果预定义数量的连续异常值被发现，质量踪迹的扩展将被取消（请参见trace_termination_outliers参数）。在'sample_rate'模式下，如果找到的峰与访问的谱图之间的比率低于'min_sample_rate'阈值，则质量踪迹在两个方向上的扩展将停止。",
    )
...
class ElutionPeakDetectionConfig(OpenMSMethodConfig):
    ...
    width_filtering: Literal["off", "fixed", "auto"] = Field(
        default="fixed",
        description="启用过滤不合理的峰宽。fixed:过滤掉超出[min_fwhm, max_fwhm]间隔的质量踪迹（相应地设置参数）。auto:使用峰宽分布的5和95百分位数进行过滤。",
    )
...
class FeatureFindingMetaboConfig(OpenMSMethodConfig):
    ...
    mz_scoring_13C: Literal["false", "true"] = Field(
        default="false",
        description="使用13C同位素峰位置（~1.003355 Da）作为同位素质量踪迹的预期偏移（强烈推荐脂质组学！）。禁用一般代谢物（如Kenar等2014，MCP中所述）。",
    )
...
class FeatureFinder(MSTool):
    ...
    def __init__(self, config: Optional[FeatureFinderConfig] = None):
