...
class SignalToNoiseConfig(OpenMSMethodParamWrapper):
    ...
    max_intensity: float = Field(
        default=-1,
        ge=-1,
        description="直方图构建所考虑的最大强度。\
            默认情况下，它将自动计算（参见 auto_mode）。\
            仅当您清楚自己在做什么（并将“auto_mode”更改为“-1”）时才提供此参数！\
            所有等于或高于“max_intensity”的强度都将被添加到最后一个直方图 bin 中。\
            如果选择的“max_intensity”太小，噪声估计也可能太小。\
            如果选择的太大，bin 将变得相当大（您可以通过增加“bin_count”来解决这个问题，但这会增加运行时）。\
            一般来说，与 MeanIterative-S/N 相比，Median-S/N 估计器对人工设置的 max_intensity 更具鲁棒性。",
    )
    ...
    auto_mode: int = Field(
        ...
        ge=-1,
        le=1,
        description="用于确定最大强度的方法：\
            -1 --> 使用“max_intensity”；\
            0 --> “auto_max_stdev_factor” 方法（默认）；\
            1 --> “auto_max_percentile” 方法",
    )
    win_len: float = Field(
...
class Centrizer(MSTool):
    ...
    def __init__(self, config: CentrizerConfig | None = None):
